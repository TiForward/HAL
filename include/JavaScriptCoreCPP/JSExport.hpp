...
class JSExport : public JSObject {
 ...
 protected:
	...
	*/
	static void AddValueProperty(const JSString& property_name, detail::GetNamedValuePropertyCallback<T> get_callback, detail::SetNamedValuePropertyCallback<T> set_callback = nullptr, bool enumerable = true);
 ...
 private:
	...
	static detail::JSClassBuilder<T> builder__;
};
...
JSExport<T>::JSExport(const JSContext& js_context)
		: JSObject(js_context, *get_js_class_ptr(), 0) {
}
